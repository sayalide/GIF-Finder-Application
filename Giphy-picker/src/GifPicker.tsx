...
import { Button, TextField, CircularProgress, Typography, Paper, Grid, Dialog, DialogTitle, DialogContent, DialogActions } from "@mui/material";
...
const GifPicker: React.FC = () => {
  ...
  const searchGifs = async (searchQuery?: string) => {
    ...
    try {
      const shouldThrottle = requestCount >= 42 || (lastRequestTimestamp && Date.now() - lastRequestTimestamp < 1000);
      if (shouldThrottle) {
        setLimitExceeded(true); // Set the limit exceeded state
        alert("API rate limit reached. Please try again in a few seconds.");
      } else {
        const response = await fetch(
        ...
        if (response.status === 429) {
          ...
          // Update the request count and last request timestamp on successful request
  ...
  return (
    <div style={{ maxWidth: 600, margin: "0 auto" }}>
      ...
      <Grid container spacing={2} style={{ marginTop: 16 }}>
        {gifs.length > 0 ? (
          gifs.map((gif) => (
            <Grid item key={gif.id} xs={12} sm={4} md={3}>
